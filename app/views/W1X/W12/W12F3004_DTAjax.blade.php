@if(count($rs))
    <div class="detailW15">
        ...
        <div class="row">
            <div class="col-md-12 col-xs-12 clsGridWidth">
                <div id="pqgrid_W12F3004" class="mg"></div>
                <table id="tblHHW12F3004">
                    <tr>
    ...
    <script type="text/javascript">
        $("#idStatusVoucher").html($('#slStatus option:selected').text());
        function resizeHeight() {
            $("#modalW12F3004").find(".detailW15").height($("#modalW12F3004").find(".modal-content").height() - $("#modalW12F3004").find(".empployeeW15").height() - 80);
            $("#pqgrid_W12F3004").pqGrid({height: $("#modalW12F3004").find(".detailW15").height() - 200});
            $("#pqgrid_W12F3004").pqGrid("refresh");
        }
        $(function () {

            $("#modalW12F3004").find(".rightContent").removeClass("hide");
            $("#modalW12F3004").find("#slEmployeeID").val("{{Session::get("W91P0000")['Creator']}}");
            $("#modalW12F3004").find("#slStatusID").val("40");
            $("#modalW12F3004").find("#txtDescription").val("");

            var summaryData = [{0:"", 1:"", 2:"", 3:"", 4:"", 5:"", 6:"{{number_format(Helpers::sumFooter($rsDetail,"Amount"),intval($rs[0]['OriginalDecimal']))}}"}];
            var tbl = $("table#tblHHW12F3004");
            ...
            $("#pqgrid_W12F3004").pqGrid(newObj);
            tbl.css("display", "none");

            //unbind t?t c? các event click tr??c ?ó c?a button
            $("#btnAppW12F3004").off("click");
            $("#btnAppW12F3004").on("click", function () {
                $.ajax({
                    method: "POST",
                    url: "{{url('W12F3004/detail/'.$rs[0]['VoucherID'])}}/" + $("#slStatus").val(),
                    data: {
                        status: $("#slStatusID").val(),
                        emp: $("#slEmployeeID").val(),
