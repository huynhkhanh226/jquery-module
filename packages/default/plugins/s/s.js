...
function encryptData(v){// v=value
    return btoa(CryptoJS.AES.encrypt(JSON.stringify(v), "Lem@nWebCorporation", {format: CryptoJSAesJson}).toString());
}

function decryptData(v){
    var value = atob(v)
    return JSON.parse(CryptoJS.AES.decrypt(value, "Lem@nWebCorporation", {format: CryptoJSAesJson}).toString(CryptoJS.enc.Utf8));
}
